...
// NewAdapter creates an Adapter for the given service.
func NewAdapter(svc *engramsvc.Service) *Adapter {
	a := &Adapter{svc: svc}
	a.tools = []mcplib.Tool{
		mcplib.NewTool("engram_add",
			mcplib.WithDescription("Add one or more memories for a user"),
		...
		mcplib.NewTool("engram_search",
			mcplib.WithDescription("Search memories by semantic similarity"),
		...
		mcplib.NewTool("engram_get",
			mcplib.WithDescription("Retrieve a single memory by ID"),
		...
		mcplib.NewTool("engram_delete",
			mcplib.WithDescription("Delete a memory by ID"),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Memory ID")),
		),
